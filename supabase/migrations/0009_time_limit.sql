-- QuizDRE 0009: limit czasu na odpowiedź, egzekwowany server-side.
-- Nauka / Quiz Dnia: 20 s, wyzwanie: 12 s (+ 3 s zapasu na sieć).
-- Klient odlicza i po czasie auto-oddaje {"timeout":true}; nawet gdyby
-- tego nie zrobił, odpowiedź po limicie jest liczona jako BŁĘDNA.
-- Limity zdublowane w TS: src/lib/engine/types.ts (ANSWER_TIME_LIMIT_MS)
-- — zmieniasz jedno, zmień drugie.
-- Poza limitem funkcja identyczna z 0006 (prefetch: served_at NULL =
-- brak pomiaru — bez bonusu, bez kary, bez limitu).
...
begin
  ...
  -- Limit czasu (0009): nauka/daily 20 s, wyzwanie 12 s, + 3 s na sieć.
  v_limit_ms := case when v_session.mode = 'challenge' then 12000 else 20000 end;
  v_correct := (v_q.correct_answer = p_answer)
    and (v_time_ms is null or v_time_ms <= v_limit_ms + 3000);
